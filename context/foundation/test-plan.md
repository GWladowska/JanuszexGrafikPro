...
| 4   | Bramki jakości w automacie                                | Wpiąć testy i `npx astro check` w CI, by czerwona zmiana nie trafiła na produkcję                         | wszystkie               | gates            | change opened | testing-quality-gates                      |
...
| hook po zapisie pliku         | lokalnie (pętla agenta)  | zalecana — wdrożona lokalnie (husky: `lint-staged` + `npm run check`) | regresje w momencie edycji                                                  |
