...
- [ ] 3.4 E2E tworzenie: świeża rejestracja → `/business/setup` → zapis → dashboard pokazuje podgląd
- [ ] 3.5 E2E edycja: konto seedowe → `/business` → zmiana nazwy i godzin; dzień zamknięty usuwa wiersz
- [ ] 3.6 Walidacje: `closes < opens` i pusta nazwa blokowane client i server
- [ ] 3.7 Bezpośrednie wejście niezalogowanego na `/business` i `/business/setup` → redirect `/auth/signin`
