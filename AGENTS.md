...
- All scripts (`dev`, `build`, `preview`, `lint`, `format`, `check`, `test`, `test:watch`, `test:integration`, `test:e2e`) are in @package.json.
...
E2E tests run on Playwright Test (`playwright.config.ts`, `e2e/**/*.spec.ts`): `npm run test:e2e` (single risk: `npm run test:e2e -- e2e/seed.spec.ts --project=chromium --reporter=line`). They need the local stack up (WSL `supabase start` + `supabase db reset`) and the dev server on `http://localhost:4321` — the config reuses a running one. Auth is a `setup` project (@e2e/auth.setup.ts) that logs in the seeded owner `owner@example.com` / `haslo12345` once and writes `playwright/.auth/user.json` (gitignored); never log in through the UI in other tests. They are **not** in CI — a deliberate scope decision.

Two E2E rules earned from real breakages:

- **Hydrate before interacting.** Before `fill`/`click` on a React island (`client:load`), call `waitForHydration(page)` from @e2e/hydration.ts (`expect(page.locator("astro-island[ssr]")).toHaveCount(0)`; Astro drops `ssr` only after hydrating — `astro/dist/runtime/server/astro-island.js:189`). Without it `fill` lands in SSR HTML and React resets the controlled input, so the test sees an empty form even though the locator looked correct.
- **A deliberate break must hit the path the assertion observes.** Week data reaches `ScheduleBoard` twice: via SSR (`initialWeekData`; the start view is the _next_ week, which the seed leaves empty) and via `GET /api/schedules` on week navigation (@src/components/schedules/ScheduleBoard.tsx:305). Breaking one does not redden a test asserting the other — if a deliberate break stays green, suspect a mis-targeted break before rewriting the assertion.
